...
/* 프로시저와 자바의 연동 
                    JAVA            ORACLE                   RETURN
   DML       CALLABLESTATEMENT     PROCEDURE                 [PARAM_NAME] OUT [DATA-TYPE]
              String dml = "{CALL [PROC_NAME]}"
              String dml = "{CALL [PROC_NAME(?,?,?)]}"
   QUERY     CALLABLESTATEMENT     PROCEDURE
              String query = "{CALL [PROC_NAME]}"
              String query = "{CALL [PROC_NAME(?,?,?)]}"     [PARAM_NAME] OUT SYS_REFCURSOR
*/
/* 매장 오픈&클로즈 프로시저 
    매장코드 IN,   직원코드 IN,    상태 IN,   결과 OUT
*/
CREATE OR REPLACE PROCEDURE STORESTATE( 
  SECODE IN SH.SH_SECODE%TYPE, 
  EMCODE IN SH.SH_EMCODE%TYPE, 
  STATUS IN INT,
  MESSAGE OUT NVARCHAR2,
  STOREINFO OUT SYS_REFCURSOR
) IS
  SHCODE    NCHAR(14);
  CK        INT;
BEGIN
    
    SELECT COUNT(*) INTO CK FROM SE WHERE SE_CODE = SECODE;
    
    IF CK = 1 THEN
        SELECT COUNT(*) INTO CK FROM EM WHERE EM_CODE = EMCODE;
        IF CK = 1 THEN
            SELECT TO_CHAR(SYSDATE, 'YYYYMMDDHH24MISS') INTO SHCODE FROM DUAL;
     
            INSERT INTO SH(SH_CODE, SH_SECODE, SH_EMCODE, SH_STATES) 
            VALUES(SHCODE, SECODE, EMCODE, STATUS);
    
            OPEN STOREINFO FOR
                SELECT SH.SH_CODE AS SDATE, 
                       SH.SH_SECODE AS SECODE, 
                       SE.SE_NAME AS SENAME, 
                       SH.SH_EMCODE AS EMCODE, 
                       "EM".EM_NAME AS EMNAME, 
                       SH.SH_STATES AS SSTATUS
                FROM SH INNER JOIN SE ON SH.SH_SECODE = SE.SE_CODE
                        INNER JOIN "EM" ON SH.SH_EMCODE = "EM".EM_CODE
                WHERE SH.SH_CODE = SHCODE AND 
                                     SH.SH_SECODE = SECODE AND 
                                     SH.SH_EMCODE = EMCODE;
        ELSE
...
SET SERVEROUTPUT ON;
VARIABLE MESSAGE NVARCHAR2;
VARIABLE STOREINFO REFCURSOR;
EXECUTE STORESTATE('10001', '10001', 9, :MESSAGE, :STOREINFO);
PRINT MESSAGE;
PRINT STOREINFO;
