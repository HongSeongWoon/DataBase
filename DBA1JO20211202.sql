...
CREATE OR REPLACE VIEW SALESINFO3
AS
SELECT 
       (ODD.ODD_QUANTITY*PR.PR_PRICE)AS TOTAMOUNT,
       (PR.PR_PRICE-PR.PR_COST) *ODD.ODD_QUANTITY AS MARGIN,
...
SELECT SS2.PRNAME AS PRNAME,
       ...
       FROM SALESINFO2 SS2 INNER OUTER JOIN OD ON SS2.ODCODE = OD.OD_CODE
                           RIGHT OUTER JOIN GO ON SS2.PRCODE=PR.PR_CODE
WHERE SUBSTR(SS2.ODCODE, 1, 8) > '20211111' AND  SUBSTR(SS2.ODCODE, 1, 8)< '20211115'
GROUP BY SS2.PRNAME;

--JOIN 할때 이렇게 넘겨주면 JOIN할때 프라이머리키는 반드시 가져가라!!!
...
SELECT CA.CAT_NAME AS CATEGORIES,
       COALESCE(SUM(ODD.ODD_QUANTITY*PR.PR_PRICE),0) AS TOTAMOUNT,
       COALESCE(SUM((PR.PR_PRICE-PR.PR_COST) *ODD.ODD_QUANTITY),0) AS MARGIN
--FROM SALESINFO3 SS3 RIGHT OUTER JOIN CA ON SS3.CATCODE = CA.CAT_CODE
       FROM ODD INNER JOIN SE ON ODD.ODD_SECODE=SE.SE_CODE
                INNER JOIN PR ON ODD.ODD_PRCODE=PR.PR_CODE
                RIGHT OUTER JOIN CA ON CA.CAT_CODE=PR.PR_CATCODE
GROUP BY CA.CAT_NAME;

SELECT*FROM CU;
     
     /* 6. 특정 고객의 특정 기간 동안 상품별 판매 정보를 출력 
    -------------------------------------------------------------------------
         상품명    판매수량     판매액    포인트적립액
    -------------------------------------------------------------------------
*/
SELECT PR.PR_NAME AS "PRNAME",
       COALESCE(SUM(ODD.ODD_QUANTITY),0) AS "SALESQUANTITY",
       COALESCE(SUM(PR.PR_PRICE*ODD.ODD_QUANTITY),0)AS "SALESAMOUNT",
       (SUM(ODD.ODD_QUANTITY * ((PR.PR_PRICE - (PR.PR_PRICE * PR_DISCOUNT)))) * 100) / 1000 AS "TOTPOINT"

FROM ODD INNER JOIN PI ON ODD.ODD_ODCODE = PI.PI_ODCODE
         RIGHT OUTER JOIN PR ON ODD_PRCODE = PR_CODE
         
WHERE PI_CUCODE = '10013' AND SUBSTR(ODD.ODD_ODCODE, 1, 8) > '20211111' AND  SUBSTR(ODD.ODD_ODCODE, 1, 8)< '20211115'
GROUP BY PR.PR_NAME;

/* 7. 특정 고객의 특정 기간 동안 상품 판매 정보를 출력 
    -------------------------------------------------------------------------
         고객명     판매액    포인트적립액  포인트사용액   포인트잔액
                  F:GO*OT    MS.POINT     MS.POINT    MS.POINT - MS.POINT
                            한컬럼을 2개로 나눠서 따로 그룹핑 하는 문제
                            최소 2개의 쿼리가 존재해야한다.
    -------------------------------------------------------------------------
*/
SELECT*FROM PI;
--고객별 일자별 판매액
CREATE OR REPLACE VIEW CUSTOMERSALES
AS
SELECT  OD.OD_CUCODE AS CUCODE, CU.CU_NAME AS CNAME, SUBSTR(ODD.ODD_ODCODE,1,8) AS SDAY,
        SUM(ODD.ODD_QUANTITY * (PR.PR_PRICE - (PR.PR_PRICE * PR_DISCOUNT)/100)) AS TOTAMOUNT
FROM ODD INNER JOIN PR ON ODD.ODD_PRCODE = PR.PR_CODE
         INNER JOIN OD ON ODD.ODD_ODCODE = OD.OD_CODE AND
                          ODD.ODD_SECODE = OD.OD_SECODE AND
                          ODD.ODD_PSCODE = OD.OD_PSCODE
        INNER JOIN CU ON OD.OD_CUCODE = CU.CU_CODE
GROUP BY OD.OD_CUCODE, CU.CU_NAME, SUBSTR(ODD.ODD_ODCODE,1,8);
...
SELECT*FROM CUSTOMERINPOINTS;
SELECT*FROM CUSTOMEROUTPOINTS;

--JOIN
SELECT CUS.CNAME AS CNAME,NVL(CUS.CUCODE, CUO.CUCODE) AS CTCODE, NVL(CUS.SDAY, CUO.PDAY) AS SDAY,COALESCE(CUS.TOTAMOUNT,0) AS TOTAMOUNT, COALESCE(CUI.INPOINTS,0) AS INPOINTS,COALESCE(CUO.OUTPOINTS,0) AS OUTPOINTS, COALESCE((CUI.INPOINTS-CUO.OUTPOINTS),0) AS REMAINPOINTS
FROM CUSTOMERSALES CUS INNER JOIN CUSTOMERINPOINTS CUI 
                               ON CUS.CUCODE = CUI.CUCODE AND CUS.SDAY=CUI.PDAY
                       FULL OUTER JOIN CUSTOMEROUTPOINTS CUO
                               ON CUS.CUCODE = CUO.CUCODE AND CUS.SDAY=CUO.PDAY
                      INNER JOIN CU ON CUS.CUCODE=CU.CU_CODE
-- 오름차순정렬 ACES은 생략가능
ORDER BY CTCODE, SDAY;
...
--INLINE VIEW
  ...
  /*내가 푼 오답.
  PI.PI_POINTS*PI.PI_STATES AS "REMAINGPOINT"

SELECT "CUNAME","SALESAMOUNT",
      ((ODD.ODD_QUANTITY * (PR.PR_PRICE - ((PR.PR_PRICE * PR_DISCOUNT) / 100))) * 100) / 1000)-(PI_POINTS*PI_STATES)) AS "USINGPOINT",
        PI_POINTS*PI_STATES AS "REMAINPOINT"
        
FROM PI LEFT OUTER JOIN(SELECT CU.CU_NAME AS "CUNAME",
                        COALESCE(SUM(PR.PR_PRICE*ODD.ODD_QUANTITY),0)AS "SALESAMOUNT",
                        (SUM(ODD.ODD_QUANTITY * (PR.PR_PRICE - ((PR.PR_PRICE * PR_DISCOUNT) / 100))) * 100) / 1000 AS "TOTPOINT",
                        PI.PI_CUCODE AS "CUCODE"
                        FROM ODD INNER JOIN PI ON ODD.ODD_ODCODE = PI.PI_ODCODE
                        RIGHT OUTER JOIN PR ON ODD_PRCODE = PR_CODE
                        INNER JOIN CU ON PI.PI_CUCODE = CU.CU_CODE
                        WHERE PI_CUCODE = '10013' AND SUBSTR(ODD.ODD_ODCODE, 1, 8) > '20211111' AND  SUBSTR(ODD.ODD_ODCODE, 1, 8)< '20211115'
                        GROUP BY CU.CU_NAME,PI.PI_CUCODE) SI 
ON PI.PI_CUCODE=SI.CUCODE;*/

/*SUB QUERY :: 쿼리안의 또다른쿼리 JOIN과 비교대상이 아님. 다른 테이블에서 데이터를 가져오는 경우
  : MAIN QUERY 안에서 실행되는 QUERY
  : FROM 절에 사용이 되면 >> INLINE VIEW
  : WHERE 절에 조건과 비교될 데이터를 다른 테이블로 부터 가져와 비교 
  : SUBQUERY 
...
/*SUB QUERY 예제
  ...
  -------------------------------------------------------------------------
   주문코드     상품명     가격      수량      합계금액
    OD, ODD      PR      PR          ODD     F:PI*ODD
  -------------------------------------------------------------------------
  */
SELECT ODD.ODD_ODCODE AS ODCODE,
       PR.PR_NAME AS PRNAME,
       PR.PR_PRICE AS PRICE,
       ODD.ODD_QUANTITY AS QUANTITY,
       PR.PR_PRICE*ODD.ODD_QUANTITY AS AMOUNT     
FROM ODD INNER JOIN PR ON ODD.ODD_PRCODE = PR.PR_CODDE;
WHERE (ODD.ODD_ODCODE, ODD.ODD_SECODE, ODD.ODD_PSCODE)
            IN(SELECT OD_CODE, OD_SECODE, OD_PSCODE
                FROM OD
                WHERE OD_CUCODE IN(SELECT CU_CODE  FROM CU WHERE CU_NAME = '김하명'));

SELECT CU_CODE FROM CU WHERE CU_NAME = '김하명' OR CU_NAME = '김철수'
;
/*하나의 테이블에 있다면 서브쿼리
여러 테이블에 있다면 JOIN*/

(SELECT COUNT(*) FROM "EM" WHERE EM_SECODE ='10001';